...
describe('MemeMinter (proxy)', function () {
  beforeEach(async function () {
    MemeMinter = await ethers.getContractFactory("DankMinter");
  ...
  it('Mint a token, expect correct URI and correct hash, expect memeId to be returned using hash', async function () {
    ...
    // create meme
    await memeMachine.createMeme(templateId, text, imgHash, URI, alice.address);
    // get info
  ...
  it('Mint token, mint another token and expect revert with cooldown error', async function () {
    ...
    await expect(memeMachine.createMeme(templateId, text, imgHash, URI2, alice.address)).to.be.revertedWith("cooldown error");
  });
  ...
  it('Mint token, update creation cooldown, mint another token and expect revert with not unique', async function () {
    ...
    await memeMachine.createMeme(templateId, text, imgHash, URI, alice.address);
    // create another meme and expect revert
    await expect(memeMachine.createMeme(templateId, text, imgHash, URI2, alice.address)).to.be.revertedWith("not unique error");
  });

  it('Mint token, update creation cooldown, mint another token and expect revert with not unique img error', async function () {
    // meme info
    const templateId = 0;
    const text = ["some text", "some more text"];
    const imgHash = ethers.utils.hexZeroPad(web3.utils.asciiToHex("12345"), 32);
    ...
    await memeMachine.createMeme(templateId, text, imgHash, URI, alice.address);
    // create another meme and expect revert
    await expect(memeMachine.createMeme(1, ["some unique text"], imgHash, URI2, alice.address)).to.be.revertedWith("not unique img error");
  });
  ...
  it('Mint token, update creation cooldown, mint another token, expect both tokens returned with getUsersMemes', async function () {
    ...
    await memeMachine.createMeme(templateId, text, imgHash, URI, alice.address);
    // create another meme and expect revert
    await memeMachine.createMeme(1, ["some unique text"], imgHash2, URI2, alice.address);
    // get users memes
